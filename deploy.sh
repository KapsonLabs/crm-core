...
PROJECT_ROOT="/root/projects/Resolver/crm-core"
VENV_NAME="crm_env"
VENV_PATH="$PROJECT_ROOT/$VENV_NAME"
MANAGE_PY="$PROJECT_ROOT/manage.py"
PROJECT_DIR="$PROJECT_ROOT"
ASGI_MODULE="crm.asgi:application"
...
#     SOCKET_FILE="/etc/systemd/system/crm.socket"
...
#     cat > "$SOCKET_FILE" << 'EOF'
# [Unit]
# Description=crm socket

# [Socket]
# ListenStream=/run/crm.sock
...
create_service_file() {
    ...
    SERVICE_FILE="/etc/systemd/system/crm.service"
...
configure_nginx() {
    ...
    NGINX_CONFIG="/etc/nginx/sites-available/crm_core"
...
server {
    listen 80;
    server_name api.resolver.iolabz.ug;
...
enable_nginx_site() {
    ...
    NGINX_AVAILABLE="/etc/nginx/sites-available/crm_core"
    NGINX_ENABLED="/etc/nginx/sites-enabled/crm_core"
...
final_status() {
    ...
    echo "   sudo systemctl status crm.socket"
    echo "   sudo systemctl status crm.service"
    ...
    echo "   sudo certbot --nginx -d api.resolver.iolabz.ug"
    echo ""
    echo -e "${GREEN}Your application should now be running!${NC}"
    echo -e "${GREEN}Admin: http://api.resolver.iolabz.ug/admin/${NC}"
    echo -e "${GREEN}API: http://api.resolver.iolabz.ug/api/${NC}"
}
